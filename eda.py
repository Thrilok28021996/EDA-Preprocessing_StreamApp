...
def EDA(uploaded_file):
    ...
    if uploaded_file is not None:
        ...
        if bar:
            fig, ax = plt.subplots()
            X_axis = st.selectbox(label='X-axis', options=['None'] + st.session_state.df.columns.tolist())
            ...
            if y_axis_option == 'value_counts':
                ...
                ax.bar(st.session_state.df[X_axis], st.session_state.df[y_axis_option], color='maroon', width=0.4)
        ...
        if line:
            ...
            if y_axis_option == 'value_counts':
                ...
                ax.plot(st.session_state.df[X_axis], st.session_state.df[y_axis_option], marker='o')
                ax.set_ylabel(y_axis_option)
